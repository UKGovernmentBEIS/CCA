...
export class InProgressTaskStatusSideEffect extends SideEffect {
  override subtask = undefined;
  override step = undefined;
  override on = ['SAVE_SUBTASK'];
  ...
  apply(payload: GenericRequestTaskPayload, subtask: string): Observable<GenericRequestTaskPayload> {
    if (!(subtask in this.subtaskToPropMap)) {
      throw new Error(
        `InProgressTaskStatusSideEffect :: You must provide a "SUBTASK_TO_PROP_MAP" containing a value for key "${subtask}"`,
      );
    }
    return of(
      produce(payload, (p) => {
        // TODO :: May need to set this dynamically
        p[this.sectionsCompletedProp][this.subtaskToPropMap[subtask]] = 'IN_PROGRESS';
      }),
    );
  }
}
