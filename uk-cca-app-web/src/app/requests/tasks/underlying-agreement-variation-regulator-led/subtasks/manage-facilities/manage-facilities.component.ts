...
export class ManageFacilitiesComponent {
  ...
  protected readonly facilityChargeStartDateMap = this.store.select(
    underlyingAgreementVariationRegulatorLedQuery.selectFacilityChargeStartDateMap,
  );
  ...
  protected readonly tableColumns: GovukTableColumn[] = [
    ...
    { field: 'links', header: 'Actions', widthClass: 'govuk-!-text-align-right' },
  ];
  ...
  hasChargeStartDate(facilityId: string): boolean {
    return this.facilityChargeStartDateMap()?.[facilityId] && this.facilityChargeStartDateMap()?.[facilityId] !== null;
  }
