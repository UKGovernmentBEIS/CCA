  ...
  onSubmit() {
    ...
    this.tasksService
      .processRequestTaskAction({
        requestTaskActionType: 'UNDERLYING_AGREEMENT_SUBMIT_PEER_REVIEW_DECISION',
        requestTaskId: +this.requestTaskStore.select(requestTaskQuery.selectRequestTaskId)(),
        requestTaskActionPayload: {
          payloadType: 'UNDERLYING_AGREEMENT_SUBMIT_PEER_REVIEW_DECISION_PAYLOAD',
          decision: decision,
        } as UnderlyingAgreementPeerReviewRequestTaskPayload,
      })
      .pipe(
        catchError((error) => {
