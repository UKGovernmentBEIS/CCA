...
export const AddTargetsFormProvider: Provider = {
  ...
  useFactory: (fb: FormBuilder, requestTaskStore: RequestTaskStore, destroyRef: DestroyRef) => {
    const targets = requestTaskStore.select(underlyingAgreementQuery.selectTargets(true))();
    ...
    improvementControl.valueChanges.pipe(takeUntilDestroyed(destroyRef)).subscribe((value) => {
      const baselineData = requestTaskStore.select(underlyingAgreementQuery.selectBaselineData(true))();

      const targetComposition = requestTaskStore.select(underlyingAgreementQuery.selectTargetComposition(true))();
      ...
      const targets =
        agreementCompositionType === 'ABSOLUTE'
          ? calculateAbsoluteTarget(energyOrCarbon, value, true)
