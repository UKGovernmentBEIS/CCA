...
import { TargetComposition, UnderlyingAgreementVariationSubmitRequestTaskPayload } from 'cca-api';
...
export class TargetCompositionComponent {
  ...
  // This is used to determine if the throughput unit is different from the sector association throughput unit
  ...
  onSubmit() {
    ...
    const updatedPayload = produce(actionPayload, (draft) => {
      draft.targetPeriod5Details.details.targetComposition = {
        calculatorFile: this.form.value.calculatorFile?.uuid ?? null,
        ...
        conversionEvidences: fileUtils.toUUIDs(this.form.value.conversionEvidences),
      };
    });

    // Apply business logic side effects for target composition changes
    const finalPayload = applyTp5TargetCompositionSideEffect(updatedPayload);
    ...
    const dto = createRequestTaskActionProcessDTO(requestTaskId, finalPayload, sectionsCompleted, reviewProps);
