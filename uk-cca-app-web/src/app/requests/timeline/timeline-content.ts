...
import { AppealOutcomeProvidedComponent } from './appeal-outcome-provided/appeal-outcome-provided.component';
...
import { EnforcementResponseNoticeSentComponent } from './enforcement-response-notice-sent/enforcement-response-notice-sent.component';
...
export const timelineContent: RequestActionPageContentFactoryMap = {
  ...
  NON_COMPLIANCE_NOTICE_OF_INTENT_SUBMITTED: () => {
    const store = inject(RequestActionStore);
    const action = store.select(requestActionQuery.selectAction)();
  ...
  NON_COMPLIANCE_ENFORCEMENT_RESPONSE_NOTICE_SUBMITTED: () => {
    const store = inject(RequestActionStore);
    const action = store.select(requestActionQuery.selectAction)();

    return {
      header: getItemActionHeader(action),
      component: EnforcementResponseNoticeSentComponent,
    };
  },

  NON_COMPLIANCE_CONCLUSION_SUBMITTED: () => {
    const store = inject(RequestActionStore);
    const action = store.select(requestActionQuery.selectAction)();

    return {
      header: getItemActionHeader(action),
      component: NonComplianceConclusionSubmittedComponent,
    };
  },

  NON_COMPLIANCE_APPEAL_PROVIDED: () => {
    const store = inject(RequestActionStore);
    const action = store.select(requestActionQuery.selectAction)();

    return {
      header: getItemActionHeader(action),
      component: NonComplianceAppealProvidedComponent,
    };
  },

  NON_COMPLIANCE_APPEAL_DETAILS_SUBMITTED: () => {
    const store = inject(RequestActionStore);
    const action = store.select(requestActionQuery.selectAction)();
  ...
  NON_COMPLIANCE_APPEAL_OUTCOME_SUBMITTED: () => {
    const store = inject(RequestActionStore);
    const action = store.select(requestActionQuery.selectAction)();

    return {
      header: getItemActionHeader(action),
      component: AppealOutcomeProvidedComponent,
    };
  },

  NON_COMPLIANCE_CLOSED: () => {
    const store = inject(RequestActionStore);
    const action = store.select(requestActionQuery.selectAction)();
  ...
  NON_COMPLIANCE_NOTICE_OF_INTENT_PEER_REVIEWER_ACCEPTED: () => {
    ...
    return {
      header: getItemActionHeader(action),
      component: NonComplianceNoticeOfIntentPeerReviewerDecisionComponent,
    };
  },
