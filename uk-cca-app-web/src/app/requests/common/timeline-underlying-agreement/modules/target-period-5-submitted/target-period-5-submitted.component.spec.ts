...
describe('TargetPeriod5Component', () => {
  let component: TargetPeriod5SubmittedComponent;
  let fixture: ComponentFixture<TargetPeriod5SubmittedComponent>;
  let store: RequestActionStore;
  let page: Page;

  class Page extends BasePage<TargetPeriod5SubmittedComponent> {
    get header() {
  ...
  it('should show summary values', () => {
    expect(page.header.textContent.trim()).toEqual('TP5 (2021-2022)');

    expect(page.summaryListValues).toEqual([
      ['Are you providing baseline and target information for TP5 (2021 to 2022)?', 'No'],
    ]);
  });
});
