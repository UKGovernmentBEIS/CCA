...
describe('MoreOrLessComponent', () => {
  ...
  @Component({
    template: '<cca-more-less-text [text]="text" [index]="index" widthClass="org-details-width"></cca-more-less-text>',
    styles: `
      .org-details-width {
  ...
  it('should implement the right id to div', () => {
    expect(element.querySelector('#more-less-link-1')).toBeNull();

    hostComponent.index = 1;
    fixture.detectChanges();

    expect(element.querySelector('#more-less-link-1')).toBeTruthy();
  });
});
