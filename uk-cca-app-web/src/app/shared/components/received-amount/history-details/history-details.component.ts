...
export class HistoryDetailsComponent {
  ...
  protected readonly amountHistory = computed(() =>
    this.receivedAmountStore.stateAsSignal()?.receivedAmountHistoryList.find((i) => i.id === this.detailsId),
  );

  protected readonly data = computed(() => toHistoryDetailsSummary(this.amountHistory()));
}
