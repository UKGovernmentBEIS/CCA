...
import { DocumentTemplateSearchResults, DocumentTemplatesService } from 'cca-api';
...
export class DocumentsComponent {
  ...
  protected readonly state = signal<DocumentTemplateSearchResults>({
    templates: [],
    total: 0,
  });
    ...
    this.activatedRoute.queryParamMap
      .pipe(
        ...
        switchMap(({ term, page, pageSize }) =>
          this.documentTemplatesService.getCurrentUserDocumentTemplates(page - 1, pageSize, [], term),
        ),
        tap(({ templates, total }) => {
          this.state.set({
            templates: templates || [],
