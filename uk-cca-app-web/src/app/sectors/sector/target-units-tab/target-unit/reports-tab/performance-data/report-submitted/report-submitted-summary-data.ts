...
import { AccountPerformanceDataReportDetailsDTO } from 'cca-api';

export function toReportSubmittedSummaryData(details: AccountPerformanceDataReportDetailsDTO) {
  ...
  factory
    ...
    .addFileListRow(
      'Uploaded files',
      fileUtils.toDownloadableFileFromInfoDTO([details?.targetPeriodReport], '../file-download'),
    )
  ...
  if (details.submissionType === 'SECONDARY') {
    factory
      ...
      .addRow(
        ...
        decimalPipe.transform(details?.secondaryMoASurplusBuyOutDTO?.priBuyOutCarbon, '1.0'),
      )
      .addRow(
