...
import { AuthoritiesService, RegulatorAuthoritiesService, RegulatorUserDTO, RegulatorUsersService } from 'cca-api';
...
describe('RegulatorDetailsComponent', () => {
  ...
  class Page extends BasePage<DetailsComponent> {
    ...
    get twoFaLink() {
      return this.query<HTMLLinkElement>('cca-two-fa-link');
    }

    get basePermissionButtonRegulatorAdmin() {
      return this.query<HTMLButtonElement>('#regulator_administrator');
    }

    get basePermissionButtonRegulatorTechnicalConsultant() {
      return this.query<HTMLButtonElement>('#regulator_technical_consultant');
    }
  }
  ...
  describe('Edit user', () => {
    ...
    it('should fill the form with predefined permissions when user role buttons are clicked', () => {
      ...
      const buttonIds = ['regulator_administrator', 'regulator_technical_consultant'];
  ...
  describe('Add new user', () => {
    ...
    it('should render header and button text for add new user', () => {
      ...
      expect(fixture.nativeElement.querySelector('button[id="regulator_administrator"]')).toBeTruthy();
    });
    ...
    it('should set aria-pressed attribute for the pressed base permission buttons when user role buttons are clicked', () => {
      ...
      expect(page.basePermissionButtonRegulatorTechnicalConsultant.getAttribute('aria-pressed')).toEqual('false');

      page.basePermissionButtonRegulatorTechnicalConsultant.click();
      fixture.detectChanges();

      expect(page.basePermissionButtonRegulatorAdmin.getAttribute('aria-pressed')).toEqual('false');
      expect(page.basePermissionButtonRegulatorTechnicalConsultant.getAttribute('aria-pressed')).toEqual('true');
