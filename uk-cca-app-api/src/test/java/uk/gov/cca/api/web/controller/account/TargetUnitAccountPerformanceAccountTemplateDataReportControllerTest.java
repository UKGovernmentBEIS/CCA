...
class TargetUnitAccountPerformanceAccountTemplateDataReportControllerTest {
	...
	@Mock
	private TargetUnitAccountPerformanceAccountTemplateDataReportServiceOrchestrator orchestrator;
	...
	void getAccountPerformanceDataStatusById_Returns200Ok() throws Exception {
		...
		when(orchestrator.getReportInfoDTO(accountId, targetPeriodType)).thenReturn(reportDTO);
		...
		verify(orchestrator, times(1)).getReportInfoDTO(accountId,
				targetPeriodType);
	}
	...
	void getAccountPerformanceAccountTemplateDataReportDetails_Returns200Ok() throws Exception {
		...
		when(orchestrator.getReportDetailsDTO(accountId, targetPeriodType)).thenReturn(reportDTO);
		...
		verify(orchestrator, times(1)).getReportDetailsDTO(accountId,
				targetPeriodType);
	}
	...
	void generateGetAccountPerformanceAccountTemplateDataReportAttachmentToken() throws Exception {
		...
		when(orchestrator.generateGetReportAttachmentToken(accountId, targetPeriodType,
				fileAttachmentUuid)).thenReturn(token);
		...
		verify(orchestrator, times(1)).generateGetReportAttachmentToken(accountId,
