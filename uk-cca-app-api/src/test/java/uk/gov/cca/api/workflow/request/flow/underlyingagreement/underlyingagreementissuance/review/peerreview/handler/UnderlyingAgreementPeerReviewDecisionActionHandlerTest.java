...
import uk.gov.cca.api.workflow.request.flow.underlyingagreement.underlyingagreementissuance.review.peerreview.service.UnderlyingAgreementPeerReviewService;
...
class UnderlyingAgreementPeerReviewDecisionActionHandlerTest {

    @InjectMocks
    private UnderlyingAgreementPeerReviewDecisionActionHandler handler;
    ...
    void process() {
        ...
        final CcaPeerReviewDecisionRequestTaskActionPayload payload =
                CcaPeerReviewDecisionRequestTaskActionPayload.builder()
                        .payloadType(CcaRequestTaskActionPayloadType.UNDERLYING_AGREEMENT_SUBMIT_PEER_REVIEW_DECISION_PAYLOAD)
        ...
        final UnderlyingAgreementPeerReviewRequestTaskPayload requestTaskPayload = UnderlyingAgreementPeerReviewRequestTaskPayload.builder()
        ...
        verify(requestService, times(1)).addActionToRequest(
                ...
                eq(CcaRequestActionType.UNDERLYING_AGREEMENT_APPLICATION_PEER_REVIEWER_REJECTED),
                eq(userId));
        ...
        assertThat(captorValue.getPayloadType()).isEqualTo(CcaRequestActionPayloadType.UNDERLYING_AGREEMENT_PEER_REVIEW_SUBMITTED_PAYLOAD);

        verify(workflowService, times(1)).completeTask(processTaskId);
        verify(validator, times(1)).validate(requestTask, payload);
        verify(underlyingAgreementPeerReviewService, times(1)).submitPeerReview(requestTask, payload);
    }

    @Test
    void getRequestTaskTypes() {
        assertThat(handler.getTypes()).containsExactly(CcaRequestTaskActionType.UNDERLYING_AGREEMENT_SUBMIT_PEER_REVIEW_DECISION);
    }
