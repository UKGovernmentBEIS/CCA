...
class SectorMoaGenerateHandlerFlowableTest {

	@InjectMocks
    private SectorMoaGenerateHandlerFlowable handler;

    @Mock
    private SectorMoaGenerateService service;
    ...
    void execute_error_occurred() throws Exception {
        ...
        verify(execution, times(1)).getVariable(BpmnProcessConstants.REQUEST_ID);
        verify(execution, times(1)).setVariable(CcaBpmnProcessConstants.SECTOR_MOA_REQUEST_ERRORS, errors);
        verify(service, times(1)).generateMoa(requestId);
    }
}
