...
import uk.gov.cca.api.workflow.request.flow.noncompliance.noticeofintent.peerreview.service.NoticeOfIntentPeerReviewService;
...
class NonComplianceNoticeOfIntentPeerReviewDecisionActionHandlerTest {

    @InjectMocks
    private NonComplianceNoticeOfIntentPeerReviewDecisionActionHandler handler;
    ...
    @Mock
    private NoticeOfIntentPeerReviewService noticeOfIntentPeerReviewService;
    ...
    void process() {
        ...
        final CcaPeerReviewDecisionRequestTaskActionPayload taskActionPayload =
                CcaPeerReviewDecisionRequestTaskActionPayload.builder()
                        .payloadType(CcaRequestTaskActionPayloadType.NON_COMPLIANCE_NOTICE_OF_INTENT_PEER_REVIEW_DECISION_PAYLOAD)
        ...
        final NonComplianceNoticeOfIntentPeerReviewRequestTaskPayload requestTaskPayload = NonComplianceNoticeOfIntentPeerReviewRequestTaskPayload.builder()
        ...
        final CcaPeerReviewDecisionSubmittedRequestActionPayload requestActionPayload = CcaPeerReviewDecisionSubmittedRequestActionPayload.builder()
                .payloadType(CcaRequestActionPayloadType.NON_COMPLIANCE_NOTICE_OF_INTENT_PEER_REVIEW_SUBMITTED_PAYLOAD)
                .decision(peerReviewDecision)
                .peerReviewAttachments(Map.of())
                .build();
        ...
        handler.process(requestTask.getId(),
                CcaRequestTaskActionType.NON_COMPLIANCE_NOTICE_OF_INTENT_SUBMIT_PEER_REVIEW_DECISION,
                appUser,
                taskActionPayload);
        ...
        verify(requestService, times(1)).addActionToRequest(
                ...
                eq(CcaRequestActionType.NON_COMPLIANCE_NOTICE_OF_INTENT_PEER_REVIEWER_REJECTED),
                eq(userId));
        ...
        assertThat(captorValue.getPayloadType()).isEqualTo(CcaRequestActionPayloadType.NON_COMPLIANCE_NOTICE_OF_INTENT_PEER_REVIEW_SUBMITTED_PAYLOAD);

        verify(requestService, times(1)).addActionToRequest(request, requestActionPayload,
                CcaRequestActionType.NON_COMPLIANCE_NOTICE_OF_INTENT_PEER_REVIEWER_REJECTED, peerReviewer);
        verify(workflowService, times(1)).completeTask(processTaskId);
        verify(noticeOfIntentPeerReviewService, times(1)).submitPeerReview(requestTask, taskActionPayload);
        verify(validator, times(1)).validate(requestTask, taskActionPayload);
    }

    @Test
    void getRequestTaskTypes() {
        assertThat(handler.getTypes()).containsExactly(CcaRequestTaskActionType.NON_COMPLIANCE_NOTICE_OF_INTENT_SUBMIT_PEER_REVIEW_DECISION);
    }
}
