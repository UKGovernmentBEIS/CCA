...
class VerifierAuthorityQueryServiceTest {
	...
	void getVerifierAuthorities_has_edit_user_scope() {
        ...
        AppAuthority appAuthority = AppAuthority.builder().verificationBodyId(vb).build();
		final AppUser authUser = AppUser.builder().userId("authUser").authorities(List.of(appAuthority)).build();
