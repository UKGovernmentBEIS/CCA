...
public class UnderlyingAgreementVariationSaveReviewDeterminationActionHandlerTest {

    @InjectMocks
    private UnderlyingAgreementVariationSaveReviewDeterminationActionHandler handler;
    ...
    void process() {
        ...
        handler.process(requestTask.getId(), CcaRequestTaskActionType.UNDERLYING_AGREEMENT_VARIATION_SAVE_REVIEW_DETERMINATION, user, reviewSaveDeterminationPayload);

        //verify
        verify(requestTaskService, times(1)).findTaskById(requestTaskId);
        verify(underlyingAgreementVariationReviewService, times(1)).saveDetermination(reviewSaveDeterminationPayload, requestTask);
    }

    @Test
    void getTypes() {
        assertThat(handler.getTypes()).containsExactly(CcaRequestTaskActionType.UNDERLYING_AGREEMENT_VARIATION_SAVE_REVIEW_DETERMINATION);
    }
}
