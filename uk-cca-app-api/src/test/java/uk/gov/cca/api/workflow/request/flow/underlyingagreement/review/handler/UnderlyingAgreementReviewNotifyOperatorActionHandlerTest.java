...
class UnderlyingAgreementReviewNotifyOperatorActionHandlerTest {
    ...
    void process() {
        ...
        final RequestTask requestTask = RequestTask.builder().id(requestTaskId).request(request).payload(taskPayload).processTaskId(processTaskId).build();
        ...
        verify(underlyingAgreementReviewValidatorService, times(1)).validate(requestTask, payload, appUser);
