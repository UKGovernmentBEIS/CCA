...
class PerformanceAccountTemplateDataUploadRequestIdGeneratorTest {

	@InjectMocks
    private PerformanceAccountTemplateDataUploadRequestIdGenerator cut;
	...
	@Test
    void getPrefix() {
        assertThat(cut.getPrefix()).isEqualTo("PATUL");
    }

    @Test
    void getTypes() {
        assertThat(cut.getTypes()).containsExactly(CcaRequestType.PERFORMANCE_ACCOUNT_TEMPLATE_DATA_UPLOAD);
    }
    ...
    void generate() {
    	...
    	String requestTypeCode = CcaRequestType.PERFORMANCE_ACCOUNT_TEMPLATE_DATA_UPLOAD;
    	...
    	RequestType requestType = RequestType.builder().code("PERFORMANCE_ACCOUNT_TEMPLATE_DATA_UPLOAD").build();
    	...
    	String expectedResult = String.format("%s-%s-%d", sectorAcronym, "PATUL", 2);
