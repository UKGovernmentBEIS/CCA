...
public class Cca3FacilityMigrationData implements Serializable {
    ...
    @CsvCustomBindByPosition(position = 7, converter = CsvBigDecimalScale7HalfDownConverter.class)
    ...
    @CsvCustomBindByPosition(position = 9, converter = CsvBigDecimalScale7HalfDownConverter.class)
    @DecimalMax(value = "100")
    @Digits(integer = 3, fraction = 7)
    private BigDecimal tp7Improvement;

    // TP8 improvement target
    @CsvCustomBindByPosition(position = 10, converter = CsvBigDecimalScale7HalfDownConverter.class)
    @DecimalMax(value = "100")
    @Digits(integer = 3, fraction = 7)
    private BigDecimal tp8Improvement;

    // TP9 improvement target
    @CsvCustomBindByPosition(position = 11, converter = CsvBigDecimalScale7HalfDownConverter.class)
    @DecimalMax(value = "100")
    @Digits(integer = 3, fraction = 7)
    private BigDecimal tp9Improvement;

    // Baseline total fixed energy (or carbon) value
    @CsvCustomBindByPosition(position = 12, converter = CsvBigDecimalScale7HalfDownConverter.class)
    @Digits(integer = Integer.MAX_VALUE, fraction = 7)
    private BigDecimal totalFixedEnergy;

    // Baseline total variable energy (or carbon)
    @CsvCustomBindByPosition(position = 13, converter = CsvBigDecimalScale7HalfDownConverter.class)
    @Digits(integer = Integer.MAX_VALUE, fraction = 7)
    private BigDecimal totalVariableEnergy;

    // Baseline Total Throughput
    @CsvCustomBindByPosition(position = 14, converter = CsvBigDecimalScale7HalfDownConverter.class)
    @Positive
    @Digits(integer = Integer.MAX_VALUE, fraction = 7)
