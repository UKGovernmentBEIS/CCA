...
public class SectorAssociationPerformanceDataReportController {
    
    private final AccountPerformanceDataStatusQueryService accountPerformanceDataStatusQueryService;
    private final SectorPerformanceDataFacilityDataReportService sectorPerformanceDataFacilityDataReportService;
    ...
    @Operation(summary = "Populates the target period performance data of sector facilities")
    @ApiResponse(responseCode = "200", description = OK, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = SectorFacilityPerformanceDataReportListDTO.class))})
    @ApiResponse(responseCode = "403", description = SwaggerApiInfo.FORBIDDEN, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))})
    @ApiResponse(responseCode = "500", description = INTERNAL_SERVER_ERROR, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))})
    @ApiResponse(responseCode = "400", description = SwaggerApiInfo.GET_SECTOR_FACILITY_PERFORMANCE_DATA_REPORT_LIST_BAD_REQUEST,
    					content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))})
    @Authorized(resourceId = "#sectorAssociationId")
    public ResponseEntity<SectorFacilityPerformanceDataReportListDTO> getSectorFacilityPerformanceDataReportList(
            @PathVariable @Parameter(description = "The sector association id") Long sectorAssociationId,
            @RequestBody @Valid @Parameter(description = "The search criteria") SectorFacilityPerformanceDataReportSearchCriteria criteria) {
            	
        return new ResponseEntity<>(
        		sectorPerformanceDataFacilityDataReportService
		                .getSectorFacilityPerformanceDataReportList(sectorAssociationId, criteria),
		        HttpStatus.OK);
    }
}
