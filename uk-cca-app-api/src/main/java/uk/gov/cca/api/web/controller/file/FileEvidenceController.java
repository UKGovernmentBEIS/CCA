...
@RestController
@RequestMapping(path = "/v1.0/file-evidences")
@RequiredArgsConstructor
@Tag(name = "File Evidences")
...
public class FileEvidenceController {
    ...
    @Operation(summary = "Get the file evidence resource for the provided file token")
    ...
    @ApiResponse(responseCode = "400", description = SwaggerApiInfo.BAD_REQUEST,
            content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = uk.gov.cca.api.web.controller.exception.ErrorResponse.class))})
    @ApiResponse(responseCode = "404", description = SwaggerApiInfo.NOT_FOUND,
            content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = uk.gov.cca.api.web.controller.exception.ErrorResponse.class))})
    @ApiResponse(responseCode = "500", description = SwaggerApiInfo.INTERNAL_SERVER_ERROR,
            content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = uk.gov.cca.api.web.controller.exception.ErrorResponse.class))})
    public ResponseEntity<Resource> getFileEvidence(
            @PathVariable("token") @Parameter(description = "The file evidence token", required = true) @NotEmpty String token) {
        FileDTO file = fileEvidenceService.getFileDTOByToken(token);
