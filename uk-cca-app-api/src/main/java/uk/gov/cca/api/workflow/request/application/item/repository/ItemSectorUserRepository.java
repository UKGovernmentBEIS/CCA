...
public class ItemSectorUserRepository {
    ...
    public ItemPage findItems(String userId, ItemAssignmentType assignmentType, Map<Long, Set<String>> scopedUserRequestTaskTypes, PagingRequest paging) {
    ...
    private Predicate constructWherePredicate(String userId, ItemAssignmentType assignmentType,
        ...
        final BooleanExpression sectorRequestTaskScopeWhereClause = CcaItemRepoUtils.constructSectorRequestTaskScopeWhereClause(
        		scopedUserRequestTaskTypes, requestTask, requestResource);

        return switch (assignmentType) {
            case ME -> requestTask.assignee.eq(userId).and(sectorRequestTaskScopeWhereClause);
            case OTHERS -> requestTask.assignee.ne(userId).and(sectorRequestTaskScopeWhereClause);
            case UNASSIGNED -> requestTask.assignee.isNull().and(sectorRequestTaskScopeWhereClause);
        };
    }
}
