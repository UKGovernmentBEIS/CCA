...
@Validated
@RestController
@RequestMapping(path = "/v1.0/target-period-reporting/sector-association")
@RequiredArgsConstructor
@Tag(name = "Sector Association Target Period Reporting")
public class SectorAccountsPerformanceReportController {

    private final AccountPerformanceDataStatusQueryService accountPerformanceDataStatusQueryService;

    @PostMapping(path = "/{sectorAssociationId}/accounts-performance-report")
    @Operation(summary = "Populates the target period performance data of sector accounts")
    @ApiResponse(responseCode = "200", description = OK, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = SectorAccountsPerformanceReportDTO.class))})
    @ApiResponse(responseCode = "403", description = SwaggerApiInfo.FORBIDDEN, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))})
    @ApiResponse(responseCode = "500", description = INTERNAL_SERVER_ERROR, content = {@Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = ErrorResponse.class))})
    @Authorized(resourceId = "#sectorAssociationId")
            public ResponseEntity<SectorAccountsPerformanceReportDTO> getSectorAccountsPerformanceDataReport(
            @PathVariable @Parameter(description = "The sector association id") Long sectorAssociationId,
            @RequestBody @Valid @Parameter(description = "The search criteria") SectorAccountsPerformanceReportSearchCriteria criteria) {

        return new ResponseEntity<>(
        		accountPerformanceDataStatusQueryService.getSectorAccountsPerformanceDataReport(sectorAssociationId, criteria), HttpStatus.OK);
    }
}