...
import uk.gov.cca.api.workflow.request.flow.performanceaccounttemplatedataupload.common.utils.PerformanceAccountTemplateUtils;
...
public class TargetPeriodEligibleAccountsQueryService {
	...
	public List<TargetUnitAccountBusinessInfoDTO> getEligibleAccountsForPerformanceAccountTemplateReporting(
		...
		final LocalDateTime terminatedDateFrom = targetPeriodYear.atMonth(Month.JANUARY).atDay(1).atTime(LocalTime.MIN);
		final LocalDateTime terminatedDateTo = PerformanceAccountTemplateUtils.TERMINATED_END_DATE_FOR_ELIGIBLE_ACCOUNTS_MONTH_DAY
				.atYear(targetPeriodYear.getValue() + 1).atTime(LocalTime.MIN);
