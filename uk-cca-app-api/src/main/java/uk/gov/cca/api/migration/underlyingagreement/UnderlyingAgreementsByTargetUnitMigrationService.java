...
public class UnderlyingAgreementsByTargetUnitMigrationService extends MigrationBaseService {
    ...
    public String getResource() {
        return "underlying-agreements-by-target-unit";
    }
    ...
    public List<String> migrate(String ids) {
        ...
        List<String> legalTargetUnits = !StringUtils.isBlank(ids)
                ? new ArrayList<>(Arrays.asList(ids.split("\\s*,\\s*")))
                : new ArrayList<>();
        
        Map<String, TargetUnitAccount> migratedAccountsToActivate = migratedTargetUnitAccountQueryService.findMigratedAccountsToActivateByTargetUnitIds(legalTargetUnits, migrationResults);
