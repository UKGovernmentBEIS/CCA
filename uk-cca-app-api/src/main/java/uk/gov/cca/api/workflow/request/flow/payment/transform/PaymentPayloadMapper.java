...
import uk.gov.cca.api.workflow.request.flow.payment.domain.PaymentProcessedRequestActionPayload;
import uk.gov.cca.api.workflow.request.flow.payment.domain.RequestPaymentInfo;
...
public interface PaymentPayloadMapper {
    ...
    @Mapping(target = "payloadType", expression = "java(uk.gov.cca.api.workflow.request.core.domain.enumeration.RequestActionPayloadType.PAYMENT_CANCELLED_PAYLOAD)")
    PaymentCancelledRequestActionPayload toPaymentCancelledRequestActionPayload(RequestPaymentInfo requestPaymentInfo);

    @Mapping(target = "payloadType", expression = "java(uk.gov.cca.api.workflow.request.core.domain.enumeration.RequestTaskPayloadType.PAYMENT_CONFIRM_PAYLOAD)")
